...
const ConversationBox: React.FC<ConversationBoxProps> = ({
  ...
  const hasSeen = useMemo(() => {
    ...
    if (seenArr?.length === 0) return false;

    return seenArr.some((user) => user.email === userEmail);
  ...
  return (
    <div
      onClick={handleClick}
      className={clsx(
        "w-full relative flex items-center space-x-3 hover:bg-neutral-100 rounded-lg transition cursor-pointer p-3 mb-1",
